...
def render_task_card(task, sm: SheetsManager):
    ...
    with col_body:
        ...
        <div class="task-card {card_cls}">
          ...
          <div class="task-badge">{badge}</div>
        </div>
        """, unsafe_allow_html=True)

    with col_actions:
        st.write("")  # チェックボックスと高さを合わせる
            ...
            with st.form("task_form", clear_on_submit=True):
                ...
                if submitted:
                    if not title.strip():
                        ...
                        if is_edit:
                            sm.update_task(eid, title.strip(), content.strip(), due_date)
                        else:
                            sm.add_task(title.strip(), content.strip(), due_date)
                        st.session_state.show_form = False
                        st.session_state.editing_id = None
                        refresh()
                        st.rerun()
...
def main():
    ...
    if view == "📅 カレンダー":
        ...
        with col_filter:
            filter_opt = st.selectbox("フィルター", ["全て", "未完了のみ", "完了のみ", "期限切れ"])
        with col_sort:
